...
const SidebarPedagogique = ({ onLogout, onNavigate, currentPath }) => {
  const [isOpen, setIsOpen] = useState(false);
  const [isMobile, setIsMobile] = useState(false);
  const [activeItem, setActiveItem] = useState(currentPath || '/pedagogique');

  // Synchroniser avec le prop currentPath
  useEffect(() => {
    if (currentPath) {
  ...
  const navigationItems = [
    ...
    {
      path: '/pedagogique/liste-professeurs',
      label: 'Professeurs',
      icon: User
    }
  ];
  ...
  return (
    <div>
      <style jsx>{`
        /* Variables CSS - Mêmes couleurs que sidebar admin */
        :root {
          --sidebar-width: 280px;
          --sidebar-bg: #ffffff;
...
export default SidebarPedagogique;