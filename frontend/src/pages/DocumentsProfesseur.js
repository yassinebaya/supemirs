...
const DocumentsProfesseur = () => {
  ...
  const fetchDocuments = async () => {
    try {
      ...
      const response = await fetch('http://195.179.229.230:5000/api2/professeur/documents', {
        headers: {
  ...
  const fetchCours = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://195.179.229.230:5000/api2/professeur/mes-cours', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const handleUpload = async (e) => {
    ...
    try {
      const response = await fetch('http://195.179.229.230:5000/api2/documents', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const handleDelete = async (documentId) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://195.179.229.230:5000/api2/documents/${documentId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`
        }
