...
const audioChunksRef = useRef([]);
  ...
  useEffect(() => {
    const fetchCurrentUser = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5000/api2/etudiant/me', {
  ...
  useEffect(() => {
    const fetchProfesseurs = async () => {
      ...
      try {
        const response = await fetch('http://195.179.229.230:5000/api2/etudiant/mes-professeurs-messages', {
          headers: { Authorization: `Bearer ${token}` }
        });
  ...
  useEffect(() => {
    const fetchMessages = async () => {
      ...
      try {
        const response = await fetch(`http://195.179.229.230:5000/api2/messages/etudiant/${selectedProf._id}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
 ...
 const envoyerMessage = async () => {
  ...
  try {
    const response = await fetch('http://195.179.229.230:5000/api2/messages/upload', {
      method: 'POST',
      headers: { Authorization: `Bearer ${token}` },
      body: formData
    });
