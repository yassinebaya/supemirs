...
const EtudiantsPedagogiquePage = () => {
  ...
  const fetchEtudiants = async () => {
    try {
      ...
      const response = await fetch('http://195.179.229.230:5000/api2/pedagogique/etudiants', {
  ...
  const fetchCommerciaux = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://195.179.229.230:5000/api2/commerciaux', {
        headers: { Authorization: `Bearer ${token}` }
      });
...
const filieresDisponibles = [...new Set(etudiants.map(e => e.filiere || e.typeFormation).filter(Boolean))].sort();
  ...
  const handleDownloadDocument = async (etudiantId, typeDocument) => {
    try {
      ...
      const response = await fetch(`http://195.179.229.230:5000/api2/etudiants/${etudiantId}/documents/${typeDocument}/download`, {
        method: 'GET',
        headers: {
  ...
  const handleValidation = async (etudiantId, statut, commentaire = '') => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`http://195.179.229.230:5000/api2/etudiants/${etudiantId}/validation-pedagogique`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
