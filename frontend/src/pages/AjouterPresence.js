    ...
    const fetchCours = async () => {
      try {
        ...
        try {
          ...
          const resCours = await axios.get('http://195.179.229.230:5000/api2/professeur/mes-cours', {
            headers: { Authorization: `Bearer ${token}` }
          });
          setCours(resCours.data);

          // ✅ Essayer de chercher la séance actuelle
          const res = await axios.get('http://195.179.229.230:5000/api2/seances/actuelle', {
            headers: { Authorization: `Bearer ${token}` }
          });
          ...
          const resEtudiants = await axios.get(`http://195.179.229.230:5000/api2/seances/${seanceActuelle._id}/etudiants`, {
            headers: { Authorization: `Bearer ${token}` }
          });
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    const resProfesseur = await axios.get('http://195.179.229.230:5000/api2/professeur/profil', {
      headers: { Authorization: `Bearer ${token}` }
    });
    ...
    if (seanceId) {
      try {
        const resSeance = await axios.get(`http://195.179.229.230:5000/api2/seances/${seanceId}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
    ...
    const promises = presences.map(pres => 
      axios.post('http://195.179.229.230:5000/api2/presences', {
