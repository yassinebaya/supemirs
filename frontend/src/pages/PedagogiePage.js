...
const PedagogiePage = () => {
  ...
  const fetchPedagogiques = async () => {
    try {
      const res = await fetch('http://195.179.229.230:5000/api2/pedagogiques', { headers });
  ...
  const handleCreateOrUpdatePedagogique = async (e) => {
    ...
    try {
      const url = editingPedagogique 
        ? `http://195.179.229.230:5000/api2/pedagogiques/${editingPedagogique._id}`
        : 'http://195.179.229.230:5000/api2/pedagogiques';
  ...
  const handleDeletePedagogique = async () => {
    ...
    setLoading(true);
    try {
      const res = await fetch(`http://195.179.229.230:5000/api2/pedagogiques/${pedagogiqueToDelete._id}`, {
        method: 'DELETE',
        headers
      });
  ...
  const handleToggleActive = async (pedagogique) => {
    try {
      setLoading(true);
      const res = await fetch(`http://195.179.229.230:5000/api2/pedagogiques/${pedagogique._id}/toggle-actif`, {
        method: 'PATCH',
        headers
      });
